  ...
  describe 'GET /api/v1/games/:id' do
    ...
    context 'Correct - 200' do
      context 'when game exists' do
        it 'returns response for game' do
          subject

          expect(json[:id]).to eq(game.id)
          expect(json[:east_id]).to eq(game.east_id)
          expect(json[:south_id]).to eq(game.south_id)
          expect(json[:west_id]).to eq(game.west_id)
          expect(json[:north_id]).to eq(game.north_id)
          expect(json[:east_score]).to eq(game.east_score)
          expect(json[:south_score]).to eq(game.south_score)
          expect(json[:west_score]).to eq(game.west_score)
          expect(json[:north_score]).to eq(game.north_score)
